...
int main() {
    // runTests();
    int nodes_number=1200;
    FlowGraph graph(nodes_number);
    std::vector<std::tuple<int, int, int>> edges;
    /*edges.push_back(std::make_tuple(0, 1, 1));
    edges.push_back(std::make_tuple(1, 7, 1));
    edges.push_back(std::make_tuple(7, 8, 1));
    edges.push_back(std::make_tuple(8, 14, 1));
    edges.push_back(std::make_tuple(14, 15, 1));
    edges.push_back(std::make_tuple(15, 16, 1));
    edges.push_back(std::make_tuple(16, 22, 1));
    edges.push_back(std::make_tuple(22, 23, 1));
    graph.AddEdges(&edges);
    //graph.DeleteEdge(3, 0);*/
    ////////////////////////////////////////////////////////////////////////
    std::ifstream in;
	//in.open("/home/bahram/Desktop/newFromScratch/a.txt");
    //in.open("/home/bahram/Desktop/joshTest/a.txt");
    in.open("a.txt");
    int capa;
    while (!in.eof())
	{
		for (int i = 0; i < nodes_number; i++) {
			for (int j = 0; j < nodes_number; j++) {
				//in >> g->capacities[IDX(i, j, 24)];
                in >> capa;
                if(capa>0){
                    edges.push_back(std::make_tuple(i, j, capa));
                }
			}
		}
	}
    ...
    auto result = graph.FlowPushRelabel("seq");
    //finalTime = CycleTimer::currentSeconds() - start;
    const double end_time = getCurrentTimestamp();
    //std::cout<<"Time:"<<finalTime * 1e3<<" ms"<<"  "<<std::endl;
    std::cout<<"Time:"<<(end_time - start_time) * 1e3<<" ms"<<"  "<<std::endl;
